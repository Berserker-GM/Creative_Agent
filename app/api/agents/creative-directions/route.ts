...
import {
  CreativeDirectionsAgentError,
  runCreativeDirectionsAgent,
} from "@/lib/agents/creative-directions";
...
export async function POST(request: Request) {
  ...
  if (!body || typeof body !== "object") {
    return errorResponse(
      400,
      "Request body must be an object with projectContext and productUnderstanding.",
    );
  }
  ...
  try {
    const creativeDirections = await runCreativeDirectionsAgent({
      projectContext: projectResult.data,
      productUnderstanding: understandingResult.data,
      referenceAnalyses,
    });

    return NextResponse.json({ creativeDirections });
  } catch (error) {
    if (error instanceof CreativeDirectionsAgentError) {
    ...
    console.error("[api/creative-directions] Unexpected failure", {
    ...
    return errorResponse(
      500,
      "Unexpected server error while generating creative directions.",
    );
  }
}
